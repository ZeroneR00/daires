...
- **Заход был пилотным: перекрашены только шапка, лента `/` и общие атомы**
  ...
  Остальные страницы (`/u/*`, `/new`, `/settings`, `/friends`, `/messages`, `/search`,
  `/login`, `/signup`, `/following`) **всё ещё на старых классах** `border-black/[.08]`,
  `bg-white`, `text-zinc-*` — на кремовом фоне они выглядят чужеродно. Это не забытый
  хвост, а осознанная граница первого шага.
