...
- **Заход был пилотным: перекрашены только шапка, лента `/` и общие атомы**
  ...
  Дальше добавились экраны состояний (2026-08-27) и страница записи
  `/u/[username]/[slug]` (2026-08-28, своя секция ниже). Остальные страницы
  (`/u/[username]` и `/u/[username]/friends`, `/new`, `/settings`, `/friends`,
  `/messages`, `/search`, `/login`, `/signup`, `/following`) **всё ещё на старых
