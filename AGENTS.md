...
- **Переключатель темы есть** (2026-09-02, своя секция ниже): «как в системе»
  по умолчанию, светлая и тёмная — явным выбором. По умолчанию тема по-прежнему
  системная, атрибута `data-theme` на `<html>` при этом нет вовсе.
...
### Переключатель темы (2026-09-02)

Последний отложенный пункт дизайн-системы. Три состояния, а не два:
**как в системе → светлая → тёмная**. Двух не хватило бы — сайт по умолчанию
следует системной настройке, и первый же клик отвязывал бы его от системы
навсегда, вернуться было бы некуда.

| Файл | Что там |
|---|---|
| `app/globals.css` | Тёмная палитра теперь достаётся двумя путями. Системная — `@media (prefers-color-scheme: dark)`, но селектор внутри стал `:root:not([data-theme="light"])`, иначе явно выбранная светлая проигрывала бы системной тёмной. Явная — `:root[data-theme="dark"]` **ниже** медиа-запроса: вес селектора тот же, выигрывает порядком. Значения тёмной палитры продублированы намеренно — CSS-переменную нельзя «унаследовать из другого селектора», а собирать её в третьем месте (класс на `<html>`) значило бы поставить цвет в зависимость от того, успел ли скрипт этот класс навесить. `color-scheme` переопределяется в обоих явных случаях — иначе нативные скроллбар и выпадашки остались бы от системной темы |
| `lib/theme.ts` | Без единого импорта, по образцу `lib/ui.ts` и `lib/realtime-channel.ts` — файл тянет и клиентская кнопка, и серверный `app/layout.tsx`. **«Как в системе» — это отсутствие атрибута, а не третье его значение:** тогда тему решает медиа-запрос, и переключать нечего. Здесь же `THEME_INIT_SCRIPT` — строка инлайн-скрипта, ключ хранилища подставляется `JSON.stringify` из той же константы, чтобы место истины было одно. Плюс свой эвент `THEME_CHANGE_EVENT`: браузерный `storage` приходит только в *другие* вкладки, и без своего события кнопка не узнала бы о собственном клике |
| `app/layout.tsx` | Инлайн-скрипт первым элементом `<body>` + `suppressHydrationWarning` на `<html>`. **Почему обычный `<script>`, а не `next/script`:** по документации (`node_modules/next/dist/docs/01-app/03-api-reference/02-components/script.md`) даже `beforeInteractive` **не блокирует отрисовку**, а против вспышки нужен именно синхронный код до первого кадра. Ставить атрибут из эффекта поздно вдвойне — React выполнит его уже после отрисовки. `suppressHydrationWarning` точечно на `<html>` и только из-за `data-theme`, ниже по дереву предупреждения работают как обычно. **Куки не рассматривались всерьёз:** сервер узнал бы тему, но layout стал бы динамическим, а сейчас `/login` и `/signup` собираются статикой (видно в выводе `next build`) |
| `components/ThemeToggle.tsx` | Своего состояния нет — читает хранилище через `useSyncExternalStore`, тем же приёмом, что `useMounted` в `SessionStatus`. Это разом снимает три вещи: нет `setState` в эффекте (на него ругается ESLint и он даёт лишний каскад), выбор подхватывается в соседних вкладках, и у React есть честный серверный снимок (`"system"`). Иконки местные, как `MenuIcon` — **фирменный знак сюда не берётся**: у него уже три роли, четвёртая превратила бы подпись в узор. Класс кнопки — тот же, что у лупы `SearchDialog`, они стоят рядом и обязаны весить одинаково |

**Что вскрылось попутно и починено в том же заходе: предсуществующее
переполнение шапки на 640–1023 px у залогиненного.** Прошлая сессия чинила
390 и мерила десктоп, а щель между ними не проверялась: порог `sm:` включал
полный ряд разделов уже на 640, где ему нужно ~936 px, и документ на 768
растягивался до 888 (замерено `git stash`-ем, до этой правки). Кнопка темы
бага не создала, но раздвинула границу до 936. Порог переехал на `lg`
(`SessionStatus`, `MobileMenu`), то есть меню-панель теперь работает до 1024.

**И вторая ловушка там же:** на 1024 ряд встаёт впритык, а имя пользователя
обрезано `max-w-[9rem]` — с именем во всю эту ширину документ вылезал на
11 px. Теперь `max-w-28 xl:max-w-36`: полные 9rem возвращаются с 1280, где
место есть. **Мерить ширину надо не на своём коротком `andrey`, а на
предельно длинном имени** — `truncate` ограничивает ширину, но обрезанный
текст всё равно занимает свой максимум.

**Проверено:** цикл переключения на живой странице; выбор переживает
перезагрузку и красит документ до отрисовки; явная светлая побеждает
системную тёмную и наоборот; в консоли ни одного предупреждения о
гидратации; `scrollWidth` равен вьюпорту на 390 / 640 / 768 / 1024 / 1100 /
1280 / 1440 и анонимом, и залогиненным, и с именем предельной длины.
