...
| `components/FriendButton.tsx` | client, по образцу `FollowButton`, но состояние не boolean, а 4 варианта; `pending_incoming` рендерит сразу две кнопки (Принять/Отклонить) вместо одной. После каждого действия зовёт `refresh()` из `useFriendRequests()` — иначе цифра в шапке не сдвинется |
...
| `components/SessionStatus.tsx` | ссылка "Друзья" → `/friends`, рядом с "Моя лента"; счётчик "Друзья (N)" берёт из `useFriendRequests()` |

**Счётчик заявок в шапке + тост-напоминание (везде) + кнопка «Позже» (на `/friends`)**
| Файл | Что там |
|---|---|
| `lib/notifications.ts` | `getFriendRequestNotice()` — **читающий** Server Action (не route handler: своих route-хендлеров в проекте нет вообще, кроме catch-all Better Auth). Параметров нет намеренно — получатель всегда владелец сессии, подставить чужой `userId` снаружи нечем, а `"use server"`-функция это публичный POST-эндпоинт. Возвращает `{ count, latestUsername, latestCreatedAt }` или `null`, считает только `ignoredAt: null`. Экспорт `interface` из `"use server"`-файла Next пропускает (типы стираются), хотя формально там разрешены только async-функции — проверено на живом дев-сервере, `tsc` это не ловит |
| `components/FriendRequestsProvider.tsx` | client-контекст + хук `useFriendRequests()` → `{ notice, announcement, refresh }`, обёрнут вокруг всего в `app/layout.tsx`. **Зачем провайдер, а не запрос внутри `SessionStatus`:** `revalidatePath` из экшена перерисовывает только серверные страницы, до клиентской шапки в layout'е не дотягивается — цифра после «Принять» замерла бы до полной перезагрузки, поэтому мутации явно зовут `refresh()`. Перечитывает при монтировании, смене `usePathname()` и `visibilitychange`; **поллинга нет намеренно** — ценой этого новая заявка замечается при первой навигации или возврате во вкладку. Внутри `refresh` — счётчик гонок через `useRef` (ответы могут прийти не в том порядке) и `.catch(() => {})`: напоминалка второстепенна, упавший запрос молчит. Сессия через `authClient.useSession()`, а не `auth.api.getSession()` в layout — та же причина, что у `SessionStatus` |
...
| `app/layout.tsx` | `<FriendRequestsProvider>` оборачивает `<Header />`, `{children}` и `<FriendRequestToast />`. Клиентский провайдер вокруг серверных потомков — нормально: они приходят пропсом `children`, а не импортируются внутрь клиентского модуля |
