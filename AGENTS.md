...
- **Переключатель темы есть** (`components/ThemeToggle.tsx`) — три состояния «как
  в системе / светлая / тёмная», по умолчанию системная, `data-theme` на `<html>`
  тогда отсутствует вовсе. Детали устройства — секция «Переключатель темы» ниже.
...
**Стена обложек смешивается умножением, а не прозрачностью** (светлая тема).
Прозрачность подмешивает бумагу в каждый пиксель обложки и тянет всё к одной
бежевой каше — отсюда была «выцветшая накладка, грязное пятно на светлом фоне».
Умножение оставляет тёмное тёмным, а светлое отпускает в цвет бумаги, плюс
тёплый `sepia(.22) saturate(1.25)` уводит стену в тон бумаги, а не в серость.
Три переменные — `--hero-blend`/`--hero-tile`/`--hero-tone` — заданы в тех же
трёх блоках палитры; **в тёмной теме умножение выключено** (`normal`, прежние
0.42): на тёмной бумаге выцветать нечему, а `screen` делал обложки молочными.
**Грабля:** `mix-blend-mode` смешивает элемент с тем, что нарисовано под ним
внутри его изолированного контекста — у `.hero-wall` (`isolate` на секции,
`-z-10`) под обложками не было ничего, и блендинг молча не работал бы; поэтому
у стены появился собственный `background: var(--paper)`. Низ стены гасит вторая
маска (`linear-gradient` + `mask-composite: intersect`) — без неё нижний ряд
обложек обрезался краем секции ровной линией, и стена читалась приклеенной
полосой. Высота своего извода — `py-12 sm:py-14` (был `py-14 sm:py-16`).
...
Три состояния — как в системе / светлая / тёмная (не два: первый клик не должен
отвязывать от системы навсегда). `app/globals.css` — тёмная палитра продублирована
намеренно (CSS-переменную нельзя унаследовать из другого селектора): системная
через `:root:not([data-theme="light"])`, явная — `:root[data-theme="dark"]` ниже
по файлу (выигрывает порядком, не весом). `lib/theme.ts` — без импортов, как
`lib/ui.ts`; «как в системе» — это отсутствие атрибута, не третье значение.
