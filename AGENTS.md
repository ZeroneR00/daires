...
- **`requireAdmin()` бросает `notFound()`, а не редирект на вход** — админка не
  палит своё существование. Граница маскировки замерена (2026-09-09): `notFound()`
  из стримящейся страницы отдаёт **HTTP 200**, так что от настоящего 404 адрес
  отличим по статусу; скрыт факт «тут закрытая страница», но не объявленный
  маршрут.
